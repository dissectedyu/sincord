...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("SINCORD_DEV_INSTALL") == "1"
	...
	SincordFile := SincordDirectory

	stat, err := os.Stat(SincordFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		SincordFile = path.Join(SincordFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(SincordFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing Sincord Install. Checking for hash...")

	re := regexp.MustCompile(`// Sincord (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(SincordDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", SincordDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", SincordDirectory+":", err)
		retErr = err
		return
	}
