...
func main() {
	...
	win = g.NewMasterWindow("Sinlotl", 1200, 800, linuxFlags)
...
func InstallLatestBuilds() (err error) {
	...
	if err != nil {
		ShowModal("Uh Oh!", "Failed to install the latest Sincord builds from GitHub:\n"+err.Error())
	}
	return
}
...
func renderInstaller() g.Widget {
	...
	layout := g.Layout{
		...
		renderErrorCard(
			...
			func() *g.MarkdownWidget {
				if cachedWarningMarkdown == nil {
					cachedWarningMarkdown = g.Markdown("**Github** and **sincord.org** are the only official places to get Sincord. Any other site claiming to be us is malicious.\n" +
			...
			g.Style().
				...
				To(
					g.Button("Reinstall / Repair").
						...
						Size(btnWidth, 50),
					Tooltip("Reinstall & Update Sincord"),
				),
			g.Style().
		...
		InfoModal("#patched", "Successfully Patched", "If Discord is still open, fully close it first.\n"+
			"Then, start it and verify Sincord installed successfully by looking for its category in Discord Settings"),
		...
		InfoModal("#scuffed-install", "Hold On!", "You have a broken Discord Install.\n"+
			...
			"You need to fix this before patching, otherwise Sincord will likely not work.\n\n"+
		...
		RawInfoModal("#openasar-confirm", "OpenAsar", "OpenAsar is an open-source alternative of Discord desktop's app.asar.\n"+
			"Sincord is in no way affiliated with OpenAsar.\n"+
	...
	g.SingleWindow().
		...
		Layout(
			g.Style().SetFontSize(baseFontSize).To(
				...
				g.Row(
					g.Label(Ternary(IsDevInstall, "Dev Install: ", "Sincord will be downloaded to: ")+SincordDirectory),
					g.Style().
						SetColor(g.StyleColorButton, DiscordBlue).
						SetStyle(g.StyleVarFramePadding, 4, 4).
						To(
							g.Button("Open Directory").OnClick(func() {
								g.OpenURL("file://" + path.Dir(SincordDirectory))
							}),
						),
				),

				&CondWidget{!IsDevInstall, func() g.Widget {
					return g.Label("To customise this location, set the environment variable 'SINCORD_USER_DATA_DIR' and restart me").Wrapped(true)
				}, nil},

				g.Dummy(0, 10),
				g.Label("Sinlotl Version: "+buildinfo.InstallerTag+" ("+buildinfo.InstallerGitHash+")"+Ternary(IsSelfOutdated, " - OUTDATED", "")),
				g.Label("Local Sincord Version: "+InstalledHash),

				&CondWidget{
					GithubError == nil,
					func() g.Widget {
						if IsDevInstall {
							return g.Label("Not updating Sincord due to being in DevMode")
						}
						return g.Label("Latest Sincord Version: " + LatestHash)
					}, func() g.Widget {
