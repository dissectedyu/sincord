...
var SincordDirectory string

var ErrAlreadyReported = errors.New("already reported")

func init() {
	if dir := os.Getenv("SINCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using SINCORD_USER_DATA_DIR")
		BaseDir = dir
	} else if dir = os.Getenv("DISCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using DISCORD_USER_DATA_DIR/../SincordData")
		BaseDir = path.Join(dir, "..", "SincordData")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("Sincord").UserConfig()
	}
	dir := os.Getenv("SINCORD_DIRECTORY")
	if dir == "" {
	...
	if dir != "" {
		Log.Debug("Using SINCORD_DIRECTORY")
		SincordDirectory = dir
	} else {
		SincordDirectory = path.Join(BaseDir, "sincord.asar")
	}
}
...
func patchAppAsar(dir string, isSystemElectron bool) (err error) {
	...
	if err := WriteAppAsar(appAsar, SincordDirectory); err != nil {
		return err
	}
...
func (di *DiscordInstall) patch() error {
	...
	if di.isFlatpak {
		...
		Log.Debug("This is a flatpak. Trying to grant the Flatpak access to", SincordDirectory+"...")
		...
		args = append(args, "override", name, "--filesystem="+SincordDirectory)
		...
		if err != nil {
			return errors.New("Failed to grant Discord Flatpak access to " + SincordDirectory + ": " + err.Error())
		}
	}
	return nil
}
...
func isSincordLoaderAppAsar(appAsar string) (bool, error) {
...
func cleanupDesyncedPatchedInstall(dir string, isSystemElectron bool) (bool, error) {
	...
	isLoader, err := isSincordLoaderAppAsar(appAsar)
	if err != nil {
		return false, err
	}
	if isLoader {
	...
	Log.Warn("Detected a patched install with a non-Sincord app.asar. Discord was likely updated while patched; removing stale _app.asar")
