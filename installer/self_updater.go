...
func GetInstallerDownloadLink() string {
	const BaseUrl = "https://github.com/Sincord/Sinlotl/releases/latest/download/"
	switch runtime.GOOS {
	case "windows":
		filename := Ternary(buildinfo.UiType == buildinfo.UiTypeCli, "SinlotlCli.exe", "Sinlotl.exe")
		return BaseUrl + filename
	case "darwin":
		switch runtime.GOARCH {
		case "amd64":
			return BaseUrl + "Sinlotl-darwin-x64.zip"
		case "arm64":
			return BaseUrl + "Sinlotl-darwin-arm64.zip"
		default:
			return ""
		}
	case "linux":
		return BaseUrl + "SinlotlCli-linux"
	default:
		return ""
	}
}
...
func UpdateSelf() error {
	...
	tmp, err := os.CreateTemp(ownExeDir, "SinlotlUpdate")
	if err != nil {
