    ...
    try {
        ...
        for (const loc of ["en-US", "en-GB", "fr-FR", navigator.language]) {
            for (const fmt of fmtSpecs) {
                try { const s = new Intl.DateTimeFormat(loc, fmt).format(d); variants.add(s); variants.add(s.replace(/\s/g, " ")); } catch { }
            }
        }
...
function getFakeDateVariants(isoDate: string): string[] {
    try {
        ...
        const variants = new Set<string>();
        for (const fmt of [
            { day: "numeric", month: "short", year: "numeric" } as Intl.DateTimeFormatOptions,
            { day: "numeric", month: "long", year: "numeric" } as Intl.DateTimeFormatOptions,
    ...
    if (storedData.createdAt) {
        ...
        if (realDates.length > 0 && fakeDates.length > 0) {
            for (const realDate of realDates) {
                if (realDate.length >= 4 && val.includes(realDate)) { result = result.split(realDate).join(fakeDates[0]); replaced = true; }
            }
        }
    }
...
function forceAccountPanelRerender() {
    try {
        ...
        const US = WP?.findByStoreName?.("UserStore");
        if (US?.emitChange) US.emitChange();
            ...
            <Menu.MenuGroup>
                {isCopied ? (
                    ...
                    <Menu.MenuItem id="copy-user-profile" label="Copy this profile" action={async () => {
                        try {
                            const targetUser = UserStore.getUser(user.id) as any;
                            ...
                            const newData: CustomProfileData = {
                                ...
                                badgeFlags: targetUser.publicFlags ?? 0, copiedUserId: user.id,
                                nitro: (profile.premiumType ?? 0) > 0,
                            };
                            try {
                                const avatarUrl = IconUtils?.getUserAvatarURL?.(targetUser, false, 512);
                                if (avatarUrl) newData.avatar = avatarUrl;
                            } catch { }
                            if (profile.banner ?? targetUser.banner) {
                                const bid = profile.banner ?? targetUser.banner;
                                newData.banner = `https://cdn.discordapp.com/banners/${user.id}/${bid}.${bid.startsWith("a_") ? "gif" : "png"}?size=512`;
                            }
        ...
        <div className="cp-field">
            ...
            <div className="cp-image-row">
                ...
                {value && <>
                    ...
                    <button className="cp-clear-btn" onClick={() => onChange("")} title="Remove"><CloseIcon /></button>
                </>}
            </div>
        </div>
    );
        ...
        <div className="cp-field">
            ...
            <div className="cp-badges">
                ...
                {NITRO_LEVELS.map((n, i) => (
                    <BadgeBtn key={i} label={n.label} icon={n.icon} active={nitroType === i} onClick={() => { onNitroType(i); if (i >= 1) onNitroType(i); }} />
                ))}
            </div>
            <div className="cp-section-label" style={{ marginTop: 8 }}>Special Badges</div>
            <div className="cp-badges">
                <BadgeBtn label="Completed a quest" icon="https://cdn.discordapp.com/badge-icons/7d9ae358c8c5e118768335dbe68b4fb8.png"
                    active={customIds.includes("quest")} onClick={() => onCustomIds(customIds.includes("quest") ? customIds.filter(x => x !== "quest") : [...customIds, "quest"])} />
                <BadgeBtn label="Orbs — Apprentice" icon="https://cdn.discordapp.com/badge-icons/83d8a1eb09a8d64e59233eec5d4d5c2d.png"
                    active={customIds.includes("orbs")} onClick={() => onCustomIds(customIds.includes("orbs") ? customIds.filter(x => x !== "orbs") : [...customIds, "orbs"])} />
            ...
            {hasOldName && (
                <div className="cp-field" style={{ marginTop: 6 }}>
                    <div className="cp-section-label">Old username displayed in tooltip</div>
                ...
                <div style={{ marginLeft: "auto", marginRight: 8, minWidth: 200 }}>
                    <Select
                        ...
                        renderOptionValue={(selected: any[]) => {
                            ...
                            if (!option) return <span>Select Account</span>;
                            return (
                                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <ModalContent className="cp-content">
                ...
                <div className="cp-field">
                    ...
                    <div className="cp-color-row" style={{ marginBottom: 6 }}>
                        ...
                        <input type="color" value={accentHex || "#5865f2"} className="cp-color-swatch"
                            onChange={e => { const n = parseInt(e.target.value.replace("#", ""), 16); if (!isNaN(n)) set("accentColor", n); }} />
                        <input value={accentHex} placeholder="#5865f2" className="cp-input cp-color-input"
                            onChange={e => { const h = e.target.value.replace("#", ""); const n = parseInt(h, 16); if (!isNaN(n) && h.length === 6) set("accentColor", n); else if (!e.target.value || e.target.value === "#") set("accentColor", undefined); }} />
                    ...
                    <div className="cp-color-row">
                        ...
                        <input type="color" value={accent2Hex || "#eb459e"} className="cp-color-swatch"
                            onChange={e => { const n = parseInt(e.target.value.replace("#", ""), 16); if (!isNaN(n)) set("accentColor2", n); }} />
                        <input value={accent2Hex} placeholder="#eb459e (optional)" className="cp-input cp-color-input"
                            onChange={e => { const h = e.target.value.replace("#", ""); const n = parseInt(h, 16); if (!isNaN(n) && h.length === 6) set("accentColor2", n); else if (!e.target.value || e.target.value === "#") set("accentColor2", undefined); }} />
                ...
                <BadgePicker
                    ...
                    nitroType={nitroLevel} onNitroType={v => { set("nitroLevel", v); if (v >= 1) set("nitro", true); }}
...
export default definePlugin({
    ...
    hookUserProfile(profile: any) {
        ...
        try {
            ...
            merged.badges = [];
            // manual merge without virtual-merge dependency
    ...
    toolboxActions: {
        "Open Profile Spoofer"() { openModal(props => <CustomProfileModal rootProps={props} />); },
    },
    ...
    userProfileBadges: [
        {
            getBadges({ userId }: { userId: string; guildId: string; }) {
                ...
                const f = storedData.badgeFlags ?? 0;
                ...
                // 4. Bug Hunter (Lvl 2 first, then Lvl 1)
                ...
                if (storedData.customBadgeIds?.includes("oldname")) {
                    const desc = storedData.oldName ? `Old username: ${storedData.oldName}` : "Old username";
                    badges.push({ id: "sp_oldname", description: desc, iconSrc: OLD_NAME_BADGE_ICON, position: 0, props: { style } });
                }
                return badges;
            }
        } as ProfileBadge
    ...
    async start() {
        ...
        try {
            ...
            if (UPS && !UPS._cp_hook) {
                ...
                UPS.getUserProfile = (uid: string) => { try { const p = origGet(uid); return (isEnabled && isMe(uid) && p) ? this.hookUserProfile(p) : p; } catch { return origGet(uid); } };
                const origGuild = UPS.getGuildMemberProfile.bind(UPS);
                UPS.getGuildMemberProfile = (uid: string, gid: string) => { try { const p = origGuild(uid, gid); return (isEnabled && isMe(uid) && p) ? this.hookUserProfile(p) : p; } catch { return origGuild(uid, gid); } };
                UPS._cp_hook = true;
            }
        } catch { }
    ...
    stop() {
        removeHeaderBarButton("profile-spoofer-btn");
    ...
    settingsAboutComponent() {
        return <Button onClick={() => openModal(props => <CustomProfileModal rootProps={props} />)}>Open Profile Spoofer</Button>;
    },
});